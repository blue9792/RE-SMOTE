...
class SMOTE(object):
    ...
    def get_syn_border_data(self):
        ...
        # 计算合成边界少数样本的数量
        border_gen_num = int(len(self.border_sample) / len(self.sample) * self.gen_num)
        # 合成的新样本
        syn_border_data = np.zeros((border_gen_num, self.feature_len))
        j = 0
        for _ in range(border_gen_num):
            # 从安全区域中随机选择一个样本点
            x = self.border_sample[random.randint(0, len(self.border_sample) - 1)]
            ...
            expectations = np.array(self.sample[x][:self.feature_len]) + gap * (
                ...
                self.sample[x][:self.feature_len]))
            # 新样本合成
            syn_border_data[j] = np.array(1 / (np.sqrt(2 * math.pi) * np.sqrt(variance)) * np.exp((-1) * ((np.array(self.sample[x][:self.feature_len]) - expectations)**2)/(2 * variance)))
            j += 1
...
if __name__ == '__main__':
    file = open('dataset/SMOTE/ecoli2/result0s0.tst', 'r')
    # 总数居集 多数类数据集 少数类数据集
    ...
    print("IR:" + str(IR))
    del_noise = Syntheic_sample.get_wenn(data, IR)
    # 去除噪声数据
