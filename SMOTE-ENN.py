...
class SMOTE(object):
    ...
    def get_enn(self):
        # 噪声样本点集合
        noise = []
        for index, single_signal in enumerate(self.sample_data):
            # 获取欧式距离
            Enclidean_distance = np.array(
                [np.sum(np.square(np.array(single_signal[:self.feature_len]) - np.array(i[:self.feature_len]))) for i in
                 self.sample_data])
            ...
            class_sum = 0
            for x in k_enn:
                class_sum += self.sample_data[x][-1]
            # 判断该样本点 所选近邻样本点的类别 此处用和代替 因为1为positive 少数类 如果最近邻多数为少数类 而自己为0 则为噪声样本
            if (class_sum / self.k > 0.5 and self.sample_data[index][-1] == 0) or (class_sum / self.k < 0.5 and
                                                                                   self.sample_data[index][-1] == 1):
                noise.append(index)
        return noise
...
if __name__ == '__main__':
    ...
    del_noise = Syntheic_sample.get_enn()
    # 去除噪声数据
